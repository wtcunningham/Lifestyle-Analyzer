...
export type ParsedTask = {
  date: Date; dayOfWeek: string; category: string; task: string;
  start?: Date | null; end?: Date | null; durationMinutes: number; comments?: string;
}

// --- Excel helpers ---
const EXCEL_EPOCH = new Date(Date.UTC(1899, 11, 30))
function excelSerialToDate(serial:number){ return new Date(EXCEL_EPOCH.getTime() + serial*24*60*60*1000) }
...
function getDowName(d:Date){ return ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][d.getDay()] }
...
function parseSheet(rows:TaskRow[]):ParsedTask[]{
  ...
  for(const r of rows){
    const nd=normalizeDate(r.Date); if(!nd) continue;
    ...
    out.push({ date: nd, dayOfWeek: r['Day Of Week']||getDowName(nd), category: (r['Task Category']||'Uncategorized').trim(), task: (r.Task||'').trim(), start: start??null, end: end??null, durationMinutes, comments: r.Comments||'' })
  }
  return out.sort((a,b)=>a.date.getTime()-b.date.getTime())
}

// --- Analytics ---
function minutesToHhMm(m:number){ const sign=m<0?'-':''; const abs=Math.abs(m); const h=Math.floor(abs/60); const mm=abs%60; return `${sign}${h}h ${mm.toString().padStart(2,'0')}m` }
function sumBy<T>(arr:T[], sel:(t:T)=>number){ return arr.reduce((acc,t)=>acc+(sel(t)||0),0) }
function groupBy<T,K extends string|number>(arr:T[], key:(t:T)=>K){ return arr.reduce((acc:any,item:T)=>{ const k=key(item); (acc[k] ||= []).push(item); return acc }, {} as Record<K,T[]>) }
function rollingStdDev(values:number[]){ if(!values.length) return 0; const mean=values.reduce((a,b)=>a+b,0)/values.length; const variance=values.reduce((a,b)=>a+(b-mean)**2,0)/values.length; return Math.sqrt(variance) }
function pickNumber<T extends Record<string, number>>(obj:T, key:string, def=0){ return typeof obj[key]==='number' ? (obj[key] as number) : def }
...
// NOTE: define the function with the exact name used below to avoid ReferenceErrors
function computeInsights(tasks: ParsedTask[]): Insights {
  ...
  const sleepPerDayMin:number[] = []
  const sleepSeries: {date:string; minutes:number}[] = []
  let totalWorkMin=0, totalHealthMin=0, exerciseDays=0
  ...
  for(const d of dates){
    ...
    const byCat = groupBy(list, (t:any)=>t.category||'Uncategorized')
    for(const cat of Object.keys(byCat)){
      const mins = sumBy(byCat[cat], (t:any)=>t.durationMinutes)
    ...
    const dayExerciseMin = sumBy(list.filter((t:any)=>EXERCISE_RE.test(t.category)||EXERCISE_RE.test(t.task)), (t:any)=>t.durationMinutes)
    if(dayExerciseMin>=20) exerciseDays += 1
    const sleepMin = sumBy(list.filter((t:any)=>/sleep/i.test(t.category)||/sleep/i.test(t.task)), (t:any)=>t.durationMinutes)
  ...
  const strengths:string[]=[], opportunities:string[]=[], redFlags:string[]=[]
...
export default function LifestyleAnalyzer(){
  ...
  const kpiBlocks = useMemo(()=>{ if(!insights) return null; const k=insights.kpis; return (<div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
    <Card className='shadow-sm'><CardHeader className='pb-2'><CardTitle className='text-lg flex items-center gap-2'><Activity className='h-5 w-5'/>Sleep</CardTitle><CardDescription>Average & consistency</CardDescription></CardHeader><CardContent><div className='text-3xl font-semibold'>{k.avgSleepHrs}h</div><div className='text-xs text-muted-foreground'>Std dev: {k.sleepConsistencyStdDevMin} min</div><div className='mt-2'><Progress value={Math.min(100,(k.avgSleepHrs/8)*100)} /></div></CardContent></Card>
    <Card className='shadow-sm'><CardHeader className='pb-2'><CardTitle className='text-lg flex items-center gap-2'><BarChart2 className='h-5 w-5'/>Work</CardTitle><CardDescription>Mean per day</CardDescription></CardHeader><CardContent><div className='text-3xl font-semibold'>{k.avgWorkHrs}h</div><div className='text-xs text-muted-foreground'>Health/Work: {k.healthVsWorkRatio}</div></CardContent></Card>
    <Card className='shadow-sm'><CardHeader className='pb-2'><CardTitle className='text-lg flex items-center gap-2'><TrendingUp className='h-5 w-5'/>Exercise</CardTitle><CardDescription>Frequency</CardDescription></CardHeader><CardContent><div className='text-3xl font-semibold'>{k.exerciseDaysPerWeek}/wk</div><div className='text-xs text-muted-foreground'>Goal: 3–5 days/week</div></CardContent></Card>
  </div>) },[insights])
  ...
  return (<div className='w-full max-w-6xl mx-auto p-4 md:p-8 bg-gradient-to-b from-white to-slate-50'>
    ...
    {!tasks && (<Card className='mb-6 border-dashed bg-gradient-to-r from-sky-50/50 to-fuchsia-50/50'>
      <CardHeader><CardTitle className='flex items-center gap-2'><Upload className='h-5 w-5'/> Upload Excel</CardTitle><CardDescription>Drag & drop your .xlsx/.xls file here, or use the button.</CardDescription></CardHeader>
      <CardContent>
    ...
    {error && (<Alert className='mb-6'><AlertTitle>Upload error</AlertTitle><AlertDescription>{error}</AlertDescription></Alert>)}

    {tasks && insights && (<div className='space-y-6'>
      <Card className='shadow-sm'>
        <CardHeader><CardTitle className='flex items-center gap-2'><Info className='h-5 w-5'/> Summary</CardTitle><CardDescription>High-level takeaways tailored from your recent days.</CardDescription></CardHeader>
        <CardContent className='space-y-4'>
          {kpiBlocks}
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4'>
            <Card className='shadow-none border'><CardHeader><CardTitle className='flex items-center gap-2'><PieIcon className='h-5 w-5'/> Time by Category (Weekdays)</CardTitle></CardHeader><CardContent style={{height:300}}><ResponsiveContainer width='100%' height='100%'><PieChart><Pie dataKey='value' data={pieDataWeekday} outerRadius={100}>{pieDataWeekday.map((_,i)=>(<Cell key={i} fill={getColor(i)} />))}</Pie><RTooltip formatter={(v:any,n:any)=>[`${v}h`,n]} /><RLegend /></PieChart></ResponsiveContainer></CardContent></Card>
            <Card className='shadow-none border'><CardHeader><CardTitle className='flex items-center gap-2'><PieIcon className='h-5 w-5'/> Time by Category (Weekends)</CardTitle></CardHeader><CardContent style={{height:300}}><ResponsiveContainer width='100%' height='100%'><PieChart><Pie dataKey='value' data={pieDataWeekend} outerRadius={100}>{pieDataWeekend.map((_,i)=>(<Cell key={i} fill={getColor(i)} />))}</Pie><RTooltip formatter={(v:any,n:any)=>[`${v}h`,n]} /><RLegend /></PieChart></ResponsiveContainer></CardContent></Card>
          </div>
          <Card className='shadow-none border'><CardHeader><CardTitle className='flex items-center gap-2'><CalendarDays className='h-5 w-5'/> Weekday vs Weekend by Category</CardTitle></CardHeader><CardContent style={{height:320}}><ResponsiveContainer width='100%' height='100%'><BarChart data={barWdWe}><CartesianGrid strokeDasharray='3 3' /><XAxis dataKey='category' hide={false} interval={0} angle={-20} textAnchor='end' height={60} /><YAxis /><RTooltip /><RLegend /><Bar dataKey='WeekdayHrs' name='Weekday (hrs)' fill='#6366F1' /><Bar dataKey='WeekendHrs' name='Weekend (hrs)' fill='#06B6D4' /></BarChart></ResponsiveContainer></CardContent></Card>
          <Card className='shadow-none border'><CardHeader><CardTitle className='flex items-center gap-2'><Activity className='h-5 w-5'/> Sleep Over Time</CardTitle></CardHeader><CardContent style={{height:280}}><ResponsiveContainer width='100%' height='100%'><LineChart data={insights.sleepSeries}><CartesianGrid strokeDasharray='3 3' /><XAxis dataKey='date' /><YAxis tickFormatter={(v)=>`${Math.round((v as number)/60)}h`} /><RTooltip formatter={(v:any)=>`${minutesToHhMm(v as number)}`} /><Line type='monotone' dataKey='minutes' name='Sleep (min)' dot={false} stroke='#10B981' /></LineChart></ResponsiveContainer></CardContent></Card>
          <div className='grid grid-cols-1 lg:grid-cols-3 gap-4'>
            <Card className='shadow-sm'><CardHeader><CardTitle className='flex items-center gap-2'><TrendingUp className='h-5 w-5'/> Strengths</CardTitle></CardHeader><CardContent><ul className='list-disc pl-5 space-y-2 text-sm'>{insights.strengths.length?insights.strengths.map((s,i)=>(<li key={i}>{s}</li>)):<li>We’ll highlight strengths once there’s enough data.</li>}</ul></CardContent></Card>
            <Card className='shadow-sm'><CardHeader><CardTitle className='flex items-center gap-2'><BarChart2 className='h-5 w-5'/> Opportunities</CardTitle></CardHeader><CardContent><ul className='list-disc pl-5 space-y-2 text-sm'>{insights.opportunities.length?insights.opportunities.map((s,i)=>(<li key={i}>{s}</li>)):<li>Opportunities for improvement will appear here.</li>}</ul></CardContent></Card>
            <Card className='shadow-sm'><CardHeader><CardTitle className='flex items-center gap-2'><AlertTriangle className='h-5 w-5'/> Red Flags</CardTitle></CardHeader><CardContent><ul className='list-disc pl-5 space-y-2 text-sm'>{insights.redFlags.length?insights.redFlags.map((s,i)=>(<li key={i}>{s}</li>)):<li>No red flags detected so far.</li>}</ul></CardContent></Card>
          </div>
        </CardContent>
      </Card>
    </div>)}

    <div className='mt-6 text-xs text-muted-foreground'><p>Tip: Categories like <em>Sleep</em>, <em>Exercise</em>, and <em>Work</em> unlock the most insights.</p></div>
  </div>)
}
