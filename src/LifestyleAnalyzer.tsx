...
function computeInsights(tasks: ParsedTask[]) {
  ...
  const weekday = { days: 0, totalsByCategory: {} as Record<string, number> };
  const weekend = { days: 0, totalsByCategory: {} as Record<string, number> };
  ...
  for (const d of dates) {
    ...
    for (const cat of Object.keys(byCat)) {
      ...
      (target.totalsByCategory as any)[cat] = ((target.totalsByCategory as any)[cat] || 0) + mins;
  ...
  return { weekday, weekend, kpis: { avgSleepHrs: +(avgSleepMin / 60).toFixed(2), sleepConsistencyStdDevMin: Math.round(sleepStd), avgWorkHrs: +(avgWorkMin / 60).toFixed(2), exerciseDaysPerWeek: +exerciseDaysPerWeek.toFixed(1), healthVsWorkRatio: +healthVsWorkRatio.toFixed(2), }, strengths, opportunities, redFlags, sleepSeries };
}
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 md:p-8 bg-gradient-to-b from-white to-slate-50">
      <div className="flex items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight flex items-center gap-2 bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-amber-500 bg-clip-text text-transparent"><span className="inline-block"><svg width='24' height='24'></svg></span> Lifestyle Analyzer</h1>
      ...
      {!tasks && (
        <Card className="mb-6 border-dashed bg-gradient-to-r from-sky-50/50 to-fuchsia-50/50">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">Upload Excel</CardTitle>
          ...
          <Card className="shadow-sm">
            ...
            <CardContent className="space-y-4">
              <Card className="shadow-none border">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">Sleep Over Time</CardTitle>
                </CardHeader>
              ...
              <Card className="shadow-none border">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">Weekday vs Weekend by Category</CardTitle>
                </CardHeader>
